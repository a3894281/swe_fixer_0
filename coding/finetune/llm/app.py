...
load_dotenv()  # Don't override existing env vars
CHUTES_ONLY = False
...
async def ainvoke_with_retry(
    ...
    chutes = Chutes(
        api_key=os.getenv("CHUTES_API_KEY"), model_timeout=180, create_chute=False
    )
    delay = initial_delay
    last_exception = None
    for attempt in range(max_retries):
        ...
        try:
            ...
            if model_exists:
                ...
                response = await chutes.invoke_async(
                    model, messages, temperature, tools, max_tokens, timeout=60
                )
                ...
                if api_key not in key_cost and os.getenv("TESTING", "false") != "true":
                    raise HTTPException(
                        status_code=400,
                        detail="The provided API key has not been initialized. Please call /init first.",
                    )
                ...
                raise Exception(
                    f"Model {model} not found in Chutes and CHUTES_ONLY is set"
                )
            ...
            if (
                isinstance(e, (openai.RateLimitError, openai.APIStatusError))
                or (isinstance(e, IndexError) and "list index out of range" in str(e))
                or "529" in str(e)
            ):  # Check common retryable error types/codes
                last_exception = e
                if attempt < max_retries - 1:
                    print(f"Retrying in {delay} seconds...")
                    await asyncio.sleep(delay)
...
async def call_llm(request: LLMRequest):
    ...
    try:
        ...
        max_tokens = models.get("gpt-4o", {}).get(
            "max_tokens", 16384
        )  # Default max_tokens
        if request.llm_name in models:
            ...
            print(
                f"Warning: llm_name '{request.llm_name}' not found in configured models. Attempting direct call."
            )
        ...
        return LLMResponse(
            ...
            total_tokens=0,
            cost=key_cost[request.api_key]
            if request.api_key in key_cost and os.getenv("TESTING", "false") != "true"
            else 0,
